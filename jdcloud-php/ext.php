...
class ExtMock implements ISmsSupport, IWxSupport, IPushMsg
{
	...
	function payToUser($weixinKey, $outTradeNo, $name, $amount, $dscr)
	{
		$log = "[企业付款给用户] openid=`{$weixinKey}`($name), partnerTradeNo=`{$outTradeNo}`, amount=`{$amount}`";
		logit($log);
		return ["payNo" => "MOCK-$outTradeNo", "payTm" => date(FMT_DT), "outTradeNo"=>$outTradeNo];
	}
...
function prepay($forJS, $outTradeNo, $amount, $dscr, $throwEx=false)
{
	try{
		$prepay = getExt(Ext_WxSupport);
		$params = $prepay->prepay($forJS, $outTradeNo, $amount, $dscr);
		return $params;
	}
	catch(Exception $e){
		logit("fail to  prepay: " . $e . "; orig msg: `$msg`");
		if ($throwEx)
			throw $e;
	}
	return false;
...
{
	try{
		$initWeixinJsapi = getExt(Ext_WxSupport);
		$params = $initWeixinJsapi->initWeixinJsapi($url);
		return $params;
	}
	catch(Exception $e){
		logit("fail to  initWeixinJsapi: " . $e . "; orig msg: `$msg`");
		if ($throwEx)
			throw $e;
	}
	return false;
